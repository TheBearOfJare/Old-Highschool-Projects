...
async def raid(message,wars,DM):
        ...
        targets = []
        for i, nation in enumerate(data["data"]["nations"]):
                inactive = nation[i]['last_active']
                ...
                if applicant == True:
                        alliance = int(nation[i]['alliance_id'])

                        for x, alliance in enumerate(alliances['data']['alliances']['data']):
                ...
                test = nation[i]['id']
                for r, wars in enumerate(nation[i]['wars']):
                    if nation[i]['wars'][r]['defid'] == test and nation[i]['wars'][r]['turnsleft'] > 0 and nation[i]['wars'][r]['defpeace'] == False:
                            def_wars+=1
                ...
                if str(nation[i]['id']) in battles or nation[i]['id'] in slotted:
                    del nation[i]
